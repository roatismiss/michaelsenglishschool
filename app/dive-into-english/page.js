  ...
  return(
    <>
      ...
      <section style={{background:"#fff",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",flexDirection:m?"column":"row",position:"relative",zIndex:2,padding:m?"36px 24px 20px":"52px 48px 36px"}}>
          <div style={{flex:m?"none":"0 0 58%",position:"relative"}}>
            {m&&<div style={{position:"absolute",top:"60px",right:0}}><RotatingAsterisk size={60}/></div>}
            <div style={{fontSize:"10px",fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase",color:C.primary,marginBottom:"10px",animation:"fadeUp 0.6s ease-out both"}}>{c.badge}</div>
            <p style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:m?"15px":"clamp(15px,1.5vw,20px)",color:C.textMuted,marginBottom:"8px",animation:"fadeUp 0.6s ease-out 0.1s both"}}>{c.tagline}</p>
            <h1 style={{fontWeight:900,fontSize:m?"clamp(2.4rem,10vw,3.2rem)":"clamp(3rem,5.5vw,5.5rem)",lineHeight:0.92,letterSpacing:"-0.03em",color:C.dark,animation:"fadeUp 0.8s ease-out 0.2s both"}}>
              {c.title[0]}<br/>{c.title[1]}<br/>{c.title[2]}
            </h1>
            <p style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:m?"16px":"clamp(17px,1.8vw,22px)",color:C.primary,marginTop:m?"12px":"16px",marginBottom:m?"18px":"24px",animation:"fadeUp 0.8s ease-out 0.35s both"}}>{c.subtitle}</p>
            <div style={{display:"flex",gap:"10px",flexWrap:"wrap",animation:"fadeUp 0.8s ease-out 0.45s both"}}>
              <a href="#contact-form" className="btn-primary" style={{display:"inline-flex",alignItems:"center",gap:"8px",padding:"12px 24px",borderRadius:"10px",fontSize:"14px",fontWeight:700,textDecoration:"none"}}>
              ...
              <a href="#includes" className="btn-outline" style={{display:"inline-flex",alignItems:"center",gap:"8px",padding:"12px 24px",borderRadius:"10px",fontSize:"14px",fontWeight:700,textDecoration:"none"}}>{c.cta2}</a>
            </div>
          </div>
          {!m&&(
            <div style={{flex:"0 0 42%",display:"flex",flexDirection:"column",justifyContent:"space-between",paddingLeft:"32px"}}>
              <div style={{display:"flex",justifyContent:"center",alignItems:"center",flex:1}}><RotatingAsterisk size={110}/></div>
              <div style={{display:"flex",alignItems:"center",gap:"10px",justifyContent:"flex-end"}}>
                <a href="/" style={{fontSize:"13px",fontWeight:600,color:C.primary,textDecoration:"none"}}>Home</a>
                <span style={{color:C.textMuted}}>/</span>
                <span style={{fontSize:"13px",fontWeight:600,color:C.text,textTransform:"uppercase",letterSpacing:"0.06em"}}>Dive Into English</span>
              </div>
            </div>
          )}
        </div>
      ...
      {/* ═══ HERO IMAGE ═══ */}
      <section style={{background:"#fff",padding:m?"20px 24px 0":"36px 48px 0"}}>
        <div style={{maxWidth:"1000px",margin:"0 auto",borderRadius:"20px",overflow:"hidden",boxShadow:"0 8px 32px rgba(67,97,238,0.1)",position:"relative"}}>
          <img src="/dive/underwater.jpeg" alt="Dive Into English" style={{width:"100%",height:m?"200px":"420px",objectFit:"cover",objectPosition:"center 35%"}}/>
          <div style={{position:"absolute",bottom:0,left:0,right:0,background:"linear-gradient(to top,rgba(26,26,46,0.85),transparent)",padding:m?"14px 18px":"22px 28px",display:"flex",gap:m?"14px":"28px",flexWrap:"wrap"}}>
            {[c.stat1,c.stat2,c.stat3].map(([n,l])=>(
              <div key={l}>
                <div style={{fontSize:m?"12px":"15px",fontWeight:800,color:"#fff"}}>{n}</div>
                <div style={{fontSize:"9px",color:"rgba(255,255,255,0.55)",letterSpacing:"0.1em",textTransform:"uppercase"}}>{l}</div>
              </div>
            ))}
          </div>
        </div>
