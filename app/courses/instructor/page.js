    ...
    <div style={{borderRadius:"16px",border:`1.5px solid ${open?C.primary:"#e4e8f2"}`,overflow:"hidden",marginBottom:"16px",transition:"border-color 0.3s"}}>
      <div onClick={onToggle} style={{padding:m?"18px 20px":"20px 28px",cursor:"pointer",background:open?C.primary:C.primaryLight,display:"flex",alignItems:"center",justifyContent:"space-between",gap:"16px",transition:"background 0.3s"}}>
        <div style={{display:"flex",alignItems:"center",gap:"14px"}}>
          ...
          <span style={{fontSize:m?"14px":"16px",fontWeight:700,color:open?"#fff":C.dark}}>{title}</span>
        </div>
...
export default function InstructorClassPage(){
  ...
  const[modOpenId,setModOpenId]=useState(1);
  const m=useMobile();
  const{t}=useLang();
    ...
    <>
      ...
      <section style={{background:"#fff",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",flexDirection:m?"column":"row",position:"relative",zIndex:2,padding:m?"40px 24px 24px":"60px 48px 40px"}}>
          <div style={{flex:m?"none":"0 0 55%",position:"relative"}}>
            {m&&<div style={{position:"absolute",top:0,right:0}}><RotatingAsterisk size={72}/></div>}
            <h1 style={{fontWeight:900,fontSize:m?"clamp(2.8rem,14vw,4.5rem)":"clamp(4rem,7vw,7rem)",lineHeight:0.92,letterSpacing:"-0.04em",color:C.dark,animation:"fadeUp 0.8s ease-out both"}}>{t("inst_hero_title_1")}<br/>{t("inst_hero_title_2")}</h1>
            <p style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:m?"20px":"clamp(22px,3vw,32px)",color:C.primary,marginTop:m?"16px":"20px",animation:"fadeUp 0.8s ease-out 0.15s both"}}>{t("inst_hero_subtitle")}</p>
          </div>
          {!m&&(
            <div style={{flex:"0 0 45%",display:"flex",flexDirection:"column",justifyContent:"space-between",paddingLeft:"40px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"12px",justifyContent:"flex-end"}}>
                ...
                <span style={{color:C.textMuted,fontSize:"14px"}}>/</span>
                <span style={{fontSize:"14px",fontWeight:600,letterSpacing:"0.08em",color:C.text,textTransform:"uppercase"}}>{t("inst_breadcrumb")}</span>
              </div>
            </div>
          )}
        </div>
        {m&&(
          <div style={{padding:"0 24px 20px",display:"flex",alignItems:"center",gap:"12px"}}>
            ...
            <span style={{color:C.textMuted,fontSize:"13px"}}>/</span>
            <span style={{fontSize:"13px",fontWeight:600,letterSpacing:"0.08em",color:C.text,textTransform:"uppercase"}}>{t("inst_breadcrumb")}</span>
          </div>
        )}
      ...
      <section style={{background:"#fff",padding:m?"32px 24px":"48px 40px"}}>
        <div style={{maxWidth:"900px",margin:"0 auto"}}>
          <div style={{borderRadius:"20px",overflow:"hidden",boxShadow:"0 8px 32px rgba(0,0,0,0.08)"}}>
            <img src="/japanese_teacher.jpg" alt="English Teacher Training" style={{width:"100%",height:m?"220px":"400px",objectFit:"cover",display:"block"}}/>
          </div>
        </div>
      </section>
        ...
        <div style={{maxWidth:"900px",margin:"0 auto",textAlign:"center"}}>
          <Reveal>
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"12px",marginBottom:"16px"}}>
              <div style={{width:"40px",height:"1.5px",background:C.primary}}/><span style={{fontSize:"11px",fontWeight:700,letterSpacing:"0.25em",textTransform:"uppercase",color:C.primary}}>{t("inst_intro_label")}</span><div style={{width:"40px",height:"1.5px",background:C.primary}}/>
            </div>
          </Reveal>
          <Reveal delay={0.1}>
            <p style={{fontFamily:"'Playfair Display',serif",fontStyle:"italic",fontSize:"14px",color:C.textMuted,marginBottom:"8px"}}>{t("inst_intro_subtitle")}</p>
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:m?"26px":"clamp(28px,5vw,40px)",fontWeight:700,color:C.dark,lineHeight:1.2,marginBottom:"28px"}}>{t("inst_intro_title")}</h2>
          </Reveal>
          <Reveal delay={0.2}>
            <p style={{fontSize:m?"15px":"17px",lineHeight:1.8,color:C.textLight,maxWidth:"700px",margin:"0 auto"}}>{t("inst_intro_desc")}</p>
          </Reveal>
        </div>
      </section>
      ...
      <section style={{background:C.primaryLight,padding:m?"48px 24px":"80px 40px"}}>
        <div style={{maxWidth:"900px",margin:"0 auto"}}>
          <Reveal>
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:m?"24px":"clamp(28px,5vw,36px)",fontWeight:700,color:C.dark,lineHeight:1.2,marginBottom:"36px"}}>{t("inst_courses_heading")}</h2>
          </Reveal>

          <Reveal>
            <AccordionItem
              num={1}
              title={t("inst_course1_title")}
              items={[]}
              extraContent={
                <p style={{fontSize:"15px",lineHeight:1.8,color:C.textLight}}>{t("inst_course1_desc")}</p>
              }
              open={openId===1}
          ...
          <Reveal delay={0.1}>
            <AccordionItem
              num={2}
              title={t("inst_course2_title")}
              items={[]}
              extraContent={
                <p style={{fontSize:"15px",lineHeight:1.8,color:C.textLight}}>{t("inst_course2_desc")}</p>
              }
              open={openId===2}
      ...
      <section style={{background:"#fff",padding:m?"48px 24px":"80px 40px"}}>
        <div style={{maxWidth:"900px",margin:"0 auto",textAlign:"center"}}>
          <Reveal>
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:m?"26px":"clamp(28px,5vw,40px)",fontWeight:700,color:C.dark,marginBottom:"12px"}}>{t("inst_clt_title")}</h2>
            <h3 style={{fontSize:m?"16px":"20px",fontWeight:700,color:C.dark,marginBottom:"28px"}}>{t("inst_clt_subtitle")}</h3>
          </Reveal>
          <Reveal delay={0.1}>
            <p style={{fontSize:"15px",lineHeight:1.8,color:C.textLight,maxWidth:"800px",margin:"0 auto 40px"}}>{t("inst_clt_desc")}</p>
          </Reveal>
        </div>
        ...
        <div style={{maxWidth:"900px",margin:"0 auto"}}>
          <Reveal>
            <h3 style={{fontSize:m?"22px":"28px",fontWeight:700,color:C.dark,marginBottom:"24px"}}>{t("inst_modules_title")}</h3>
          </Reveal>

          <Reveal>
            <AccordionItem
              num={"C"}
              title={t("inst_mod_core_title")}
              items={[t("inst_mod_core_1"),t("inst_mod_core_2"),t("inst_mod_core_3")]}
              open={modOpenId===1}
              onToggle={()=>setModOpenId(modOpenId===1?null:1)}
              m={m}
            />
          </Reveal>
          ...
          <Reveal delay={0.05}>
            <AccordionItem
              num={"S"}
              title={t("inst_mod_spec_title")}
              items={[t("inst_mod_spec_1"),t("inst_mod_spec_2")]}
              open={modOpenId===2}
              onToggle={()=>setModOpenId(modOpenId===2?null:2)}
              m={m}
            />
          </Reveal>
          ...
          <Reveal delay={0.1}>
            <AccordionItem
              num={"A"}
              title={t("inst_mod_apply_title")}
              items={[t("inst_mod_apply_1"),t("inst_mod_apply_2"),t("inst_mod_apply_3"),t("inst_mod_apply_4")]}
              open={modOpenId===3}
              onToggle={()=>setModOpenId(modOpenId===3?null:3)}
              m={m}
            />
          </Reveal>
        </div>
      ...
      <section style={{background:C.primary,padding:m?"48px 24px":"64px 40px"}}>
        <div style={{maxWidth:"900px",margin:"0 auto"}}>
          <Reveal>
            <h2 style={{fontSize:m?"28px":"36px",fontWeight:800,color:"#fff",marginBottom:"12px"}}>{t("inst_eft_title")}</h2>
            <h3 style={{fontSize:m?"20px":"24px",fontWeight:700,color:"rgba(255,255,255,0.85)",marginBottom:"32px"}}>{t("inst_eft_subtitle")}</h3>
          </Reveal>
          <Reveal delay={0.1}>
            <div style={{display:"flex",flexDirection:"column",gap:"20px"}}>
              <div style={{display:"flex",gap:"12px",alignItems:"flex-start"}}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2.5" style={{flexShrink:0,marginTop:"3px"}}><path d="M20 6L9 17l-5-5"/></svg>
                <p style={{fontSize:"15px",lineHeight:1.8,color:"rgba(255,255,255,0.9)"}}>{t("inst_eft_point1")}</p>
              </div>
              <div style={{display:"flex",gap:"12px",alignItems:"flex-start"}}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2.5" style={{flexShrink:0,marginTop:"3px"}}><path d="M20 6L9 17l-5-5"/></svg>
                <p style={{fontSize:"15px",lineHeight:1.8,color:"rgba(255,255,255,0.9)"}}>{t("inst_eft_point2")}</p>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      <section style={{background:"#fff",padding:m?"48px 24px":"64px 40px",textAlign:"center"}}>
        <Reveal>
          <p style={{fontSize:"15px",lineHeight:1.8,color:C.textLight,maxWidth:"600px",margin:"0 auto 32px"}}>{t("inst_cta_ielts")}</p>
        </Reveal>
        <Reveal delay={0.1}>
